# Generate-NutanixAlertsAndIndex.ps1 v4.1 - Dec 01, 2025
#
...
#
# v4.1:
# - Added a new robust helper function 'Parse-NcliDate' to reliably parse date strings
#   by first removing timezone abbreviations (PDT, EST, UTC) and then trying multiple
#   exact format strings. This should finally resolve the date parsing errors.
#
...
function Parse-NcliDate {
    ...
    # 'dd' handles two-digit days (e.g., 26), 'd' handles single-digit days (e.g., 5).
    # The double space after MMM handles inconsistent spacing from ncli output.
    $dateFormats = @(
        "ddd MMM dd HH:mm:ss yyyy", # e.g., "Fri Sep 26 11:40:44 2025"
        "ddd MMM  d HH:mm:ss yyyy"  # e.g., "Fri Sep  5 11:40:44 2025"
    )

    foreach ($format in $dateFormats) {
        try {
            # Attempt to parse the sanitized string with the current exact format.
            return [datetime]::ParseExact($sanitizedDateString, $format, $null)
        } catch {
            # If it fails, continue to the next format in the list.
            continue
        }
    }
    ...
    if ($alerts.Count -gt 0) {
        ...
        foreach ($group in $groupedAlerts) {
            ...
            foreach($alert in $group.Group){
                ...
                try {
                    # --- FIX v4.1: Use the robust helper function to parse the date ---
...
$reportHtmlFoot = @"
<footer>Script: Generate-NutanixAlertsAndIndex.ps1 v4.1</footer></body></html>
"@
...
$indexHtmlFoot = @"
<footer>Index generated by Generate-NutanixAlertsAndIndex.ps1 v4.1</footer>
"@
