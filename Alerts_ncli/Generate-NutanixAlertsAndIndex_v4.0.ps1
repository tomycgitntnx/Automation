# Generate-NutanixAlertsAndIndex.ps1 v4.0 - Dec 01, 2025
#
...
#
# v4.0:
# - Branched to new version number for a stable release.
# - Implemented a robust date-parsing method by removing timezone strings (e.g., EDT, UTC)
#   before parsing to prevent errors and ensure "Days Old" calculates correctly.
# - Removed conditional color-coding for the "Created On" date.
#
    ...
    if ($alerts.Count -gt 0) {
        ...
        foreach ($group in $groupedAlerts) {
            ...
            foreach($alert in $group.Group){
                ...
                try {
                    # FIX: Remove timezone string (e.g., EDT, UTC) and then parse the date
                    $sanitizedDateString = $alert.CreatedOnStr -replace ' [A-Z]{3} ', ' '
                    $createdOnDate = [datetime]::Parse($sanitizedDateString)
...
$reportHtmlFoot = @"
<footer>Script: Generate-NutanixAlertsAndIndex.ps1 v4.0</footer></body></html>
"@
...
$indexHtmlFoot = @"
<footer>Index generated by Generate-NutanixAlertsAndIndex.ps1 v4.0</footer>
"@
