# Generate-NutanixAlertsAndIndex.ps1 v2.5 - Nov 26, 2025
#
...
foreach ($namecluster in $clusters) {
    ...
    if ($alerts.Count -gt 0) {
        ...
        foreach ($group in $groupedAlerts) {
            ...
            # --- Change 2: Add 'Days Old' column header ---
            $groupTable = "<table><tr><th>Severity</th><th>Message</th><th>Created On</th><th>Days Old</th></tr>"

            foreach($alert in $group.Group){
                # --- Change 1: Corrected logic to assign date color class ---
                # --- Change 2: Calculate 'Days Old' value ---
                $ageClass = ""
                $daysOld = ""
                try {
                    $createdOnDate = [datetime]::Parse($alert.CreatedOnStr)
                    $alertAge = $reportTime - $createdOnDate
                    $daysOld = [math]::Floor($alertAge.TotalDays) # Get the whole number of days

                    if ($alertAge.TotalDays > 30) {
                        $ageClass = "age-old"
                    } 
                    elseif ($alertAge.TotalDays >= 1 -and $alertAge.TotalDays -le 7) {
                        $ageClass = "age-week"
                    } 
                    elseif ($alertAge.TotalHours < 24) {
                        $ageClass = "age-new"
                    }
                } catch {} # Silently ignore parsing errors

                $severityClass = "count-$($alert.Severity)".ToLower()
                # --- Change 2: Add 'Days Old' data to the table row ---
...
"@
$reportHtmlFoot = @"
<footer>Script: Generate-NutanixAlertsAndIndex.ps1 v2.5</footer></body></html>
"@
...
$indexHtmlFoot = @"
<footer>Index generated by Generate-NutanixAlertsAndIndex.ps1 v2.5</footer></body></html>
"@
