# Generate-NutanixAlertsAndIndex.ps1 v2.6 - Nov 26, 2025
#
...
#
# v2.6 Updates:
# - Fixed the recurring PowerShell parsing error in the if/elseif statement by
#   enclosing the entire conditional expression in parentheses.
#
# v2.5 Updates:
# - Fixed the conditional logic for time-based color-coding of the "Created On" date.
# - Added a new "Days Old" column to the alert details table to show alert age.
#
    ...
    if ($alerts.Count -gt 0) {
        ...
        foreach ($group in $groupedAlerts) {
            ...
            foreach($alert in $group.Group){
                # --- FIX v2.6: Corrected the if/elseif logic to prevent parsing errors ---
                $ageClass = ""
                $daysOld = ""
                try {
                    ...
                    elseif (($alertAge.TotalDays -ge 1) -and ($alertAge.TotalDays -le 7)) {
                        $ageClass = "age-week"
                    }
...
"@
$reportHtmlFoot = @"
<footer>Script: Generate-NutanixAlertsAndIndex.ps1 v2.6</footer></body></html>
"@
...
$indexHtmlFoot = @"
<footer>Index generated by Generate-NutanixAlertsAndIndex.ps1 v2.6</footer></body></html>
"@
