# Generate-NutanixAlertsAndIndex.ps1 v2.1 - Nov 26, 2025
#
...
#
# v2.1 Updates:
# - Severity color-coding now applies to text only (not background).
# - Cells with a '0' count for alerts are not color-coded.
# - Changed cluster "launch" link to an up-arrow symbol (↗) next to the cluster name.
# - Moved the "Back to Summary" link to the right side of the cluster summary header.
#
...
<style>
    ...
    /* Severity Color-Coding (Text Only) */
...
foreach ($namecluster in $clusters) {
    ...
    # --- Change 2 & 3: Add launch symbol and reposition 'Back to Summary' link ---
    ...
    if ($alerts.Count -gt 0) {
        ...
        foreach ($group in $groupedAlerts) {
            ...
            foreach($alert in $group.Group){
                # --- Change 1: Apply class only to text, not background ---
...
if($allClustersSummaryData.Count -gt 0){
    ...
    foreach($summary in $allClustersSummaryData){
        # --- Change 2: Add launch symbol link to cluster name in summary ---
        ...
        # --- Change 1: Conditionally apply color class based on count ---
        $criticalCell = if ($summary.Critical -gt 0) { "<td class='severity-Critical'>$($summary.Critical)</td>" } else { "<td>$($summary.Critical)</td>" }
        $warningCell  = if ($summary.Warning -gt 0)  { "<td class='severity-Warning'>$($summary.Warning)</td>" }   else { "<td>$($summary.Warning)</td>" }
        $infoCell     = if ($summary.Info -gt 0)     { "<td class='severity-Info'>$($summary.Info)</td>" }         else { "<td>$($summary.Info)</td>" }
...
"@
$reportHtmlFoot = @"
<footer>Script: Generate-NutanixAlertsAndIndex.ps1 v2.1</footer></body></html>
"@
...
$indexHtmlFoot = @"
<footer>Index generated by Generate-NutanixAlertsAndIndex.ps1 v2.1</footer></body></html>
"@
