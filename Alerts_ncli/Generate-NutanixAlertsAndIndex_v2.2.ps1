# Generate-NutanixAlertsAndIndex.ps1 v2.2 - Nov 26, 2025
#
...
#
# v2.2 Updates:
# - Added dynamic, time-based color-coding for the 'Created On' date field.
# - Updated the 'Alert Summary' table to use a new color palette for severity counts.
#
...
<style>
    ...
    /* --- Change 2: New color classes for Summary Counts --- */
    ...
    /* --- Change 1: New color classes for Alert Age --- */
    ...
    if ($alerts.Count -gt 0) {
        ...
        foreach ($group in $groupedAlerts) {
            ...
            foreach($alert in $group.Group){
                # --- Change 1: Logic to determine age class for 'Created On' date ---
                $ageClass = ""
                try {
                    $createdOnDate = [datetime]::Parse($alert.CreatedOnStr)
                    $alertAge = $reportTime - $createdOnDate
                    if ($alertAge.TotalDays > 30) { $ageClass = "age-old" }
                    elseif ($alertAge.TotalDays >= 1) { $ageClass = "age-week" }
                    elseif ($alertAge.TotalHours < 24) { $ageClass = "age-new" }
                } catch {} # Silently ignore parsing errors

                $severityClass = "count-$($alert.Severity)".ToLower() # Use summary count colors for severity text
...
if($allClustersSummaryData.Count -gt 0){
    ...
    foreach($summary in $allClustersSummaryData){
        ...
        # --- Change 2: Conditionally apply new color classes based on count ---
...
"@
$reportHtmlFoot = @"
<footer>Script: Generate-NutanixAlertsAndIndex.ps1 v2.2</footer></body></html>
"@
...
$indexHtmlFoot = @"
<footer>Index generated by Generate-NutanixAlertsAndIndex.ps1 v2.2</footer></body></html>
"@
