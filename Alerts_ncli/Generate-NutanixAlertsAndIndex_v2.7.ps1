# Generate-NutanixAlertsAndIndex.ps1 v2.7 - Nov 26, 2025
#
...
#
# v2.7 Updates:
# - Fixed the DateTime parsing error by switching from Parse() to ParseExact() and
#   providing the correct custom format string for the 'Created On' date.
# - This resolves the empty "Days Old" column and enables time-based color-coding.
#
    ...
    if ($alerts.Count -gt 0) {
        ...
        foreach ($group in $groupedAlerts) {
            ...
            foreach($alert in $group.Group){
                ...
                $daysOld = ""
                try {
                    # --- FIX v2.7: Use ParseExact with the correct format for the ncli date string ---
                    $dateFormat = "ddd MMM dd HH:mm:ss 'UTC' yyyy"
                    $createdOnDate = [datetime]::ParseExact($alert.CreatedOnStr, $dateFormat, $null)
                    $alertAge = $reportTime - $createdOnDate
                    $daysOld = [math]::Floor($alertAge.TotalDays)

                    if ($alertAge.TotalHours -lt 24) {
                        $ageClass = "age-new"
                    } 
                    elseif ($alertAge.TotalDays -le 7) {
                        $ageClass = "age-week"
                    }
                    elseif ($alertAge.TotalDays -gt 30) {
                        $ageClass = "age-old"
                    }
                } catch {
                ...
                $groupTable += "<tr><td class='$severityClass'>$($alert.Severity)</td><td>$($alert.Message)</td><td class='$ageClass'>$($alert.CreatedOnStr)</td><td>$daysOld</td></tr>"
            }
...
"@
$reportHtmlFoot = @"
<footer>Script: Generate-NutanixAlertsAndIndex.ps1 v2.7</footer></body></html>
"@
...
$indexHtmlFoot = @"
<footer>Index generated by Generate-NutanixAlertsAndIndex.ps1 v2.7</footer></body></html>
"@
