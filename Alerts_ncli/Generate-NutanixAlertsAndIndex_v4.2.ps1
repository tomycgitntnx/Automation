# Generate-NutanixAlertsAndIndex.ps1 v4.2 - Dec 01, 2025
#
...
#
# v4.2:
# - Implemented a new, more robust `Parse-NcliDate` function specifically for PowerShell 5.1.
#   It removes any timezone abbreviation (e.g., PDT, EST, UTC) before using ParseExact
#   with multiple formats to handle date string variations reliably.
#
...
# --- Helper Function for Robust Date Parsing (PowerShell 5.1 Compatible) ---
function Parse-NcliDate {
    param(
        [string]$DateString
    )

    # First, remove the problematic timezone abbreviation (e.g., " PDT ", " EST ", " UTC ").
    # This regex looks for a space, three capital letters, and a space, and replaces it with a single space.
    $sanitizedDateString = $DateString -replace ' [A-Z]{3} ', ' '

    # Define an array of possible date formats to try.
    # 'dd' handles two-digit days (e.g., 25), 'd' handles single-digit days (e.g., 5).
    # The double space after MMM handles inconsistent spacing from ncli output (e.g., "Sep  5").
    $dateFormats = @(
        "ddd MMM dd HH:mm:ss yyyy", # e.g., "Thu Sep 25 11:34:14 2025"
        "ddd MMM  d HH:mm:ss yyyy"  # e.g., "Thu Sep  5 11:34:14 2025"
    )

    foreach ($format in $dateFormats) {
        try {
            # Attempt to parse the sanitized string with the current exact format.
            # Using [System.Globalization.CultureInfo]::InvariantCulture ensures consistency.
            return [datetime]::ParseExact($sanitizedDateString, $format, [System.Globalization.CultureInfo]::InvariantCulture)
        } catch {
            # If it fails, silently continue to the next format in the list.
            continue
        }
    }

    # If all formats fail, throw an exception to be caught by the main script logic.
...
foreach ($namecluster in $clusters) {
    ...
    $allClustersHtmlBody += "<summary><span class='cluster-title'>$namecluster <a class='launch-link' href='https://$($namecluster):9440' target='_blank'>&#x2197;</a></span><a class='back-to-summary' href='#summary'>Back to Summary</a></summary>"
    ...
    if ($alerts.Count -gt 0) {
        ...
        foreach ($group in $groupedAlerts) {
            ...
            $groupTable = "<table><tr><th>Severity</th><th>Message</th><th>Created On</th><th>Days Old</th></tr>"

            foreach($alert in $group.Group){
                $daysOld = "N/A" # Default value
                try {
                    # Use the robust helper function to parse the date
                    $createdOnDate = Parse-NcliDate -DateString $alert.CreatedOnStr

                    $alertAge = $reportTime - $createdOnDate
                    $daysOld = [math]::Floor($alertAge.TotalDays)
                } catch {
                    # If parsing fails, Days Old remains "N/A".
                    Write-Warning "Could not parse date: $($alert.CreatedOnStr). Error: $($_.Exception.Message)"
                }

                $severityClass = "count-$($alert.Severity)".ToLower()
                # The 'Created On' TD no longer has a color class
                $groupTable += "<tr><td class='$severityClass'>$($alert.Severity)</td><td>$($alert.Message)</td><td>$($alert.CreatedOnStr)</td><td>$daysOld</td></tr>"
            }
...
if($allClustersSummaryData.Count -gt 0){
    ...
    foreach($summary in $allClustersSummaryData){
        $clusterLink = "<a href='#cluster-$($summary.ClusterName)'>$($summary.ClusterName)</a> <a class='launch-link' href='https://$($summary.ClusterName):9440' target='_blank'>&#x2197;</a>"
...
$reportHtmlFoot = @"
<footer>Script: Generate-NutanixAlertsAndIndex.ps1 v4.2</footer></body></html>
"@
...
$indexHtmlFoot = @"
<footer>Index generated by Generate-NutanixAlertsAndIndex.ps1 v4.2</footer>
"@
