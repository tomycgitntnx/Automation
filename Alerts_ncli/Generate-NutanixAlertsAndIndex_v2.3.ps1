# Generate-NutanixAlertsAndIndex.ps1 v2.3 - Nov 26, 2025
#
...
#
# v2.3 Updates:
# - Fixed a PowerShell parsing error in the if/elseif block for alert age calculation.
# - Made the conditional logic for date comparison more robust.
#
# v2.2 Updates:
    ...
    if ($alerts.Count -gt 0) {
        ...
        foreach ($group in $groupedAlerts) {
            ...
            foreach($alert in $group.Group){
                # --- FIX v2.3: Corrected the if/elseif logic to avoid parsing errors ---
                $ageClass = ""
                try {
                    ...
                    if ($alertAge.TotalHours -lt 24) { 
                        $ageClass = "age-new" 
                    } elseif ($alertAge.TotalDays -lt 7) { 
                        $ageClass = "age-week" 
...
"@
$reportHtmlFoot = @"
<footer>Script: Generate-NutanixAlertsAndIndex.ps1 v2.3</footer></body></html>
"@
...
$indexHtmlFoot = @"
<footer>Index generated by Generate-NutanixAlertsAndIndex.ps1 v2.3</footer></body></html>
"@
