# Generate-NutanixAlertsAndIndex.ps1 v3.1 - Dec 01, 2025
#
...
#
# v3.1 Updates:
# - Fixed the date parsing issue by removing the timezone abbreviation (e.g., EDT, EST, UTC)
#   from the date string before parsing. This makes the function reliable.
#
...
function Parse-NcliDate {
    ...
    # --- FIX v3.1: Remove the timezone string (e.g., " EDT ", " UTC ") before parsing ---
    # This is more reliable than trying to replace it.
    $sanitizedDateString = $DateString -replace ' [A-Z]{3} ', ' '

    try {
        # Parse the string without the problematic timezone abbreviation.
        # PowerShell will treat it as local time, which is the desired behavior.
        return [datetime]::Parse($sanitizedDateString)
    }
    catch {
        # If parsing still fails for an unknown reason, throw an exception.
        throw "Unable to parse date string: '$DateString' even after sanitizing."
    }
}
            ...
            foreach($alert in $group.Group){
                ...
                try {
                    ...
                    $daysOld = [math]::Floor($alertAge.TotalDays)
                     ...
                     # If parsing fails, Days Old remains N/A. A warning is logged to the console.
...
"@
$reportHtmlFoot = @"
<footer>Script: Generate-NutanixAlertsAndIndex.ps1 v3.1</footer></body></html>
"@
...
$indexHtmlFoot = @"
<footer>Index generated by Generate-NutanixAlertsAndIndex.ps1 v3.1</footer></body></html>
"@
