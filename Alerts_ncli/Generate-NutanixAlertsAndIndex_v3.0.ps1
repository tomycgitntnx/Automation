# Generate-NutanixAlertsAndIndex.ps1 v3.0 - Dec 01, 2025
#
...
#
# v3.0 Updates:
# - Made the Parse-NcliDate function timezone-agnostic. It now uses a regular expression
#   to handle any three-letter timezone (e.g., UTC, EDT, PST) from ncli output.
#
# v2.9 Updates:
# - Removed the time-based color-coding logic for the "Created On" date field.
#
...
function Parse-NcliDate {
    ...
    # --- FIX v3.0: Make the parsing timezone-agnostic ---
    # The ncli command can output different timezones (e.g., UTC, EDT, PST).
    # This regex finds any three-letter, all-caps timezone abbreviation surrounded by spaces.
    if ($DateString -match ' ([A-Z]{3}) ') {
        # Replace the found timezone with a standard offset that PowerShell can parse.
        # This makes the function work regardless of the timezone reported by the cluster.
        $genericDateString = $DateString -replace ' [A-Z]{3} ', ' +00:00 '

        # Define formats that PowerShell can now parse.
        # 'd' handles single-digit days, 'dd' handles two-digit days.
        $dateFormats = @(
        ...
        foreach ($format in $dateFormats) {
            try {
                return [datetimeoffset]::ParseExact($genericDateString, $format, $null).DateTime
            } catch {
                continue
            }
        }
    ...
    # If the above logic fails for any reason, fall back to the original method.
    try {
        $legacyDateFormats = @(
            "ddd MMM dd HH:mm:ss 'UTC' yyyy",
            "ddd MMM  d HH:mm:ss 'UTC' yyyy"
        )
        foreach ($format in $legacyDateFormats) {
            try {
    ...
    if ($alerts.Count -gt 0) {
        ...
        foreach ($group in $groupedAlerts) {
            ...
            foreach($alert in $group.Group){
                ...
                try {
                    # Use the robust parsing function to get the date
                    ...
                    $daysOld = [math]::Floor($alertAge.TotalDays)
                     ...
                     # If parsing fails, Days Old remains N/A. Warning is logged to console.
                ...
                # The 'Created On' TD no longer has an age-based color class
...
"@
$reportHtmlFoot = @"
<footer>Script: Generate-NutanixAlertsAndIndex.ps1 v3.0</footer></body></html>
"@
...
$indexHtmlFoot = @"
<footer>Index generated by Generate-NutanixAlertsAndIndex.ps1 v3.0</footer></body></html>
"@
