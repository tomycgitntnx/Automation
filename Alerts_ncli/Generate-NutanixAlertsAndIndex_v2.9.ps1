# Generate-NutanixAlertsAndIndex.ps1 v2.9 - Dec 01, 2025
#
...
#   The date will now be displayed with normal text color.
#
# v2.8 Updates:
# - Introduced a robust `Parse-NcliDate` function to handle multiple possible date formats.
# - Updated the cluster launch link to be dynamic based on the cluster name.
#
...
function Parse-NcliDate {
    ...
    # Define an array of possible date formats from ncli.
    $dateFormats = @(
        "ddd MMM dd HH:mm:ss 'UTC' yyyy", # Format for two-digit days (e.g., "Fri Sep 19...")
        "ddd MMM  d HH:mm:ss 'UTC' yyyy"  # Format for single-digit days with extra space (e.g., "Mon Dec  1...")
    )
    ...
    if ($alerts.Count -gt 0) {
        ...
        foreach ($group in $groupedAlerts) {
            ...
            foreach($alert in $group.Group){
                ...
                # The 'Created On' TD no longer has an $ageClass variable
...
"@
$reportHtmlFoot = @"
<footer>Script: Generate-NutanixAlertsAndIndex.ps1 v2.9</footer></body></html>
"@
...
$indexHtmlFoot = @"
<footer>Index generated by Generate-NutanixAlertsAndIndex.ps1 v2.9</footer></body></html>
"@
