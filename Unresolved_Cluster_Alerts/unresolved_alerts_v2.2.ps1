...
function Generate-HtmlReports {
    ...
    # --- HTML and CSS Style (New style integrated with required table/color styles) ---
    $htmlStyle = @"
<style>
    ...
    /* Styles preserved from previous version for tables and colors */
    ...
    .summary-table tr:nth-child(even), .alerts-table tr:nth-child(even) { background-color: #f8f9fa; }
...
"@
    ...
    foreach ($monthGroup in $groupedReports) {
        $indexBody += "<details><summary>$($monthGroup.Name)</summary><div><ul>"
...
if ($allAlerts.Count -gt 0) {
    ...
    # Even if no alerts, we can update the index page
    Generate-HtmlReports -AllAlerts @() -ReportsDirectory $reportsDir
}
