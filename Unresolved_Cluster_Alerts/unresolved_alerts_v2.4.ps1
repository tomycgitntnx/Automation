...
function Get-UnresolvedNutanixAlerts {
    ...
    try {
        ...
        # This response has no 'data' property.
...
"@
    ...
    foreach ($group in $groupedAlerts) {
        ...
        $detailsHtml += "<table class='alerts-table'><tr><th>Severity</th><th>Title</th><th>Created Time</th><th>Entity</th><th>Message</th><th>Root Cause Analysis</th></tr>"
        foreach ($alert in $group.Group) {
            ...
            # Format Root Cause Analysis for better readability
            $rcaFormatted = ""
            if ($alert.RootCauseAnalysis) {
                if ($alert.RootCauseAnalysis.cause) {
                    $rcaFormatted += "Cause: $($alert.RootCauseAnalysis.cause)<br>"
                }
                if ($alert.RootCauseAnalysis.resolution) {
                    $rcaFormatted += "Resolution: $($alert.RootCauseAnalysis.resolution)"
                }
            }

            $detailsHtml += "<tr class='$severityClass'><td>$($alert.Severity)</td><td>$($alert.Title)</td><td>$($alert.CreatedTime)</td><td>$($alert.SourceEntity)</td><td>$message</td><td>$rcaFormatted</td></tr>"
        }
