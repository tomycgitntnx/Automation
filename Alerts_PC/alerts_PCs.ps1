...
Function Get-CredentialSecure {
    ...
    if (Test-Path $CredentialFile) {
        try {
            # Use a static key for portability or generate one per user profile.
            # This key must be present on any machine running the script if the cred file is moved.
            $keyPath = Join-Path $env:USERPROFILE "key.key"
            if (-not (Test-Path $keyPath)) {
                New-SecureString -String "your-secret-key-phrase-123456" | ConvertFrom-SecureString | Out-File -FilePath $keyPath
            }
