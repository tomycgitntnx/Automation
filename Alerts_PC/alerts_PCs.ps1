...
.DESCRIPTION
    ...
    - Securely stores and reuses credentials with a correctly formatted byte-array key.
...
.EXAMPLE
    ...
    .\get_alerts_pc.ps1 -PCAddress "pc1.mydomain.local", "192.168.10.100"
...
Function Get-CredentialSecure {
    ...
    # The -Key parameter requires a byte array of 16, 24, or 32 bytes.
    # We will generate a 256-bit (32-byte) key.
    if (-not (Test-Path $keyPath)) {
        Write-Host "Encryption key file not found. Creating a new one at '$keyPath'."
    ...
    if (Test-Path $CredentialFile) {
        try {
            # Read the key as a byte array
            $key = [System.IO.File]::ReadAllBytes($keyPath)
            $Password = Get-Content $CredentialFile | ConvertTo-SecureString -Key $key
            return New-Object System.Management.Automation.PSCredential($Username, $Password)
        }
        catch {
            Write-Warning "Could not decrypt the password file (perhaps the key changed). Please re-enter the password."
        }
    }
    ...
    # Read the key as a byte array and save the encrypted password
    $key = [System.IO.File]::ReadAllBytes($keyPath)
...
Function Get-NutanixUnresolvedAlerts {
    ...
    if ($null -eq $Credential) {
        Write-Error "Credential object is null. Halting connection to $PC."
        return $null
    }
...
<style>
    ...
    table { border-collapse: collapse; width: 100%; margin-top: 20px; border: 1px solid #ddd; table-layout: fixed; }
...
if ($allClustersData.Count -gt 0) {
    ...
    foreach ($clusterName in $allClustersData.Keys.GetEnumerator() | Sort-Object) {
        ...
        $table = $allClustersData[$clusterName].Alerts | Sort-Object { [datetime]$_.creationTime } -Descending | ForEach-Object {
            $cause = if ($_.rootCauseAnalysis) { ($_.rootCauseAnalysis.cause -join " ") | ForEach-Object {$_ -replace '<', '&lt;' -replace '>', '&gt;'} } else { "N/A" }
            $resolution = if ($_.rootCauseAnalysis) { ($_.rootCauseAnalysis.resolution -join " ") | ForEach-Object {$_ -replace '<', '&lt;' -replace '>', '&gt;'} } else { "N/A" }
            $message = if ($_.message) { $_.message | ForEach-Object {$_ -replace '<', '&lt;' -replace '>', '&gt;'} } else { "N/A" }
...
foreach ($group in $groupedReports) {
    ...
    foreach ($report in $group.Group) {
        # Ensure path uses forward slashes for HTML compatibility
        $relativePath = (Join-Path $ReportsFolder $report.Name) -replace '\\', '/'
