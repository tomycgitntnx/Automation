# Create-NCC-ReportIndex.ps1 v1.3  Oct 07, 2025
#
...
# v1.1 - Removed the weekly grouping to simplify the layout. Reports are now listed directly under each month.
# v1.2 - Original script provided for modification.
# v1.3 - Modified HTML anchor tags to remove 'target="_blank"' so links open in the same tab.
#
# Usage:
...
# Group all reports by Month-Year (e.g., "September 2025") and sort newest first
...
foreach ($monthGroup in $groupedByMonth) {
    ...
    $htmlBody += "<details open><summary><h2>$($monthGroup.Name)</h2></summary><ul>"

    # List each report run directly within that month
    foreach ($report in $monthGroup.Group) {
        ...
        $htmlBody += "<a href='$reportSummaryPath'>View Summary Report</a>"
        $htmlBody += "<a href='$($report.RelativePath)/'>Browse Files</a>"
        $htmlBody += "</span>"
        $htmlBody += "</li>"
    }
    $htmlBody += "</ul></details>"
...
$htmlFoot = @"
    <footer>Index generated by Create-NCC-ReportIndex.ps1 v1.3</footer>
</body>
</html>
"@
