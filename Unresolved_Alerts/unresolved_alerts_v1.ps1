...
function Get-AlertEntities {
    ...
    # New API format: sourceEntity.name
    if ($Alert.PSObject.Properties.Match('sourceEntity')) {
        $sourceEntity = $Alert.sourceEntity
        if ($null -ne $sourceEntity) {
            if ($sourceEntity.PSObject.Properties.Match('name')) {
                $name = $sourceEntity.name
                if ($null -ne $name -and -not [string]::IsNullOrWhiteSpace($name)) {
    ...
    # Fallback to old format support
...
function Get-AlertCause {
    param (
        [Parameter(Mandatory)]
        $Alert
    )

    if ($Alert.PSObject.Properties.Match('rootCauseAnalysis')) {
        $rca = $Alert.rootCauseAnalysis
        if ($null -ne $rca) {
            $rcaList = @($rca) | Where-Object { $_ }
            if ($rcaList.Count -gt 0) {
                $firstRca = $rcaList[0]
                if ($null -ne $firstRca -and $firstRca.PSObject.Properties.Match('cause')) {
                    $cause = $firstRca.cause
                    if ($null -ne $cause -and -not [string]::IsNullOrWhiteSpace($cause)) {
...
function Get-AlertResolution {
    ...
    if ($Alert.PSObject.Properties.Match('rootCauseAnalysis')) {
        ...
        if ($null -ne $rca) {
            ...
            if ($rcaList.Count -gt 0) {
                $firstRca = $rcaList[0]
                if ($null -ne $firstRca -and $firstRca.PSObject.Properties.Match('resolution')) {
                    $resolution = $firstRca.resolution
                    if ($null -ne $resolution -and -not [string]::IsNullOrWhiteSpace($resolution)) {
...
function Get-AlertCreatedTime {
    ...
    # New API format: creationTime (ISO 8601 string)
    $createdTime = Get-AlertTimestamp -Alert $Alert -PropertyNames @('creationTime', 'created_time_stamp_in_usecs', 'created_time_usecs', 'creation_timestamp_in_usecs', 'creation_time', 'created_time')
    return $createdTime
}
...
function Get-AlertLastUpdatedTime {
    ...
    # New API format: lastUpdatedTime (ISO 8601 string)
    $lastUpdatedTime = Get-AlertTimestamp -Alert $Alert -PropertyNames @('lastUpdatedTime', 'last_updated_time_stamp_in_usecs', 'last_occurrence_time_usecs', 'last_updated_time', 'update_time')
...
function Invoke-NutanixAlertsQuery {
    ...
    $apiVersion = 'v4.0.b1'
    $baseUri = "https://$Endpoint`:9440/api/monitoring/$apiVersion/serviceability/alerts"
    $filter = 'isResolved eq false'
    $select = 'title,severity,creationTime,lastUpdatedTime,rootCauseAnalysis,sourceEntity/name'
    
    # Build the URI with query parameters
    $uri = "$baseUri" + "?`$filter=" + [System.Uri]::EscapeDataString($filter) + "&`$select=" + [System.Uri]::EscapeDataString($select)
    ...
    try {
        ...
        while ($true) {
            ...
            $currentUri = if ($skip -gt 0) { "$uri&`$skip=$skip&`$top=$pageSize" } else { "$uri&`$top=$pageSize" }
            
            $response = Invoke-NutanixRestMethod -Uri $currentUri -Credential $Credential -Method 'Get'
            ...
            if ($response -is [System.Array]) {
                $alerts = @($response) | Where-Object { $_ }
            }
            elseif ($response.PSObject.Properties.Match('data')) {
                $alerts = @($response.data) | Where-Object { $_ }
            }
            elseif ($response.PSObject.Properties.Match('entities')) {
                $alerts = @($response.entities) | Where-Object { $_ }
            }

            $collectedAlerts += $alerts

            # Check if we got fewer results than requested (end of data)
            if ($alerts.Count -lt $pageSize) {
                break
            }
        ...
        Write-Log -Level 'ERROR' -Message "Alerts query failed for '$Endpoint': $($_.Exception.Message)"
        throw "Unable to retrieve alerts from '$Endpoint' using API version '$apiVersion': $($_.Exception.Message)"
    }
}
...
function New-DetailPageHtml {
    ...
    foreach ($alert in $Alerts) {
        ...
        $name = ConvertTo-HtmlEncoded -InputObject (Get-AlertEntities -Alert $alert)
        $title = ConvertTo-HtmlEncoded -InputObject (Get-AlertTitle -Alert $alert)
        $cause = ConvertTo-HtmlEncoded -InputObject (Get-AlertCause -Alert $alert)
        $resolution = ConvertTo-HtmlEncoded -InputObject (Get-AlertResolution -Alert $alert)
...
"@) | Out-Null
    ...
    if ($rowsBuilder.Length -eq 0) {
        $rowsBuilder.AppendLine('<tr><td colspan="7" class="empty">No unresolved alerts.</td></tr>') | Out-Null
    }

    $header = ConvertTo-HtmlEncoded -InputObject $Endpoint
    $clusterUrl = "https://$Endpoint`:9440"
    $clusterUrlEncoded = ConvertTo-HtmlEncoded -InputObject $clusterUrl
    $headerWithLink = "<a href=""$clusterUrlEncoded"" target=""_blank"">$header</a>"
...
<head>
    ...
    <style>
        ...
        h1 a:hover { text-decoration: underline; color: #1565c0; }
...
<body>
    ...
    <h1>Alerts for $headerWithLink</h1>
    <ul class="meta">
...
function New-DashboardHtml {
    ...
    foreach ($row in $SummaryRows) {
        ...
        $clusterUrl = "https://$($row.Endpoint):9440"
        $clusterUrlEncoded = ConvertTo-HtmlEncoded -InputObject $clusterUrl
        
        # Create link to cluster web interface
        $clusterLink = "<a href=""$clusterUrlEncoded"" target=""_blank"">$endpoint</a>"
        
        # Add link to detail file if it exists
        $detailLink = if ($row.DetailFile) { " <a href=""$($row.DetailFile)"" title=""View alert details"">[Details]</a>" } else { '' }
        ...
        $rowsBuilder.AppendLine(@"
            <tr>
                <td>$clusterLink$detailLink $statusBadge $errorHtml</td>
